...
import { buildScenarioStepNumbers, countAttachments, countDescendants, formatDuration, formatSize, hasStepContent, normalizeScenario } from './scenarioModel';
...
function ScenarioTreeNode({ step, path, stepNumbers }) {
  const [expanded, setExpanded] = useState(false);
    ...
    <div className="scenario-tree-node" data-testid="scenario-step" data-scenario-path={path} data-step-number={stepNumbers.get(path)}>
      ...
      {hasDetails && (
        <div hidden={!expanded} className="scenario-tree-children" data-testid="scenario-step-details">
          ...
          {subSteps.map((child, index) => <ScenarioTreeNode step={child} path={`${path}.${index}`} stepNumbers={stepNumbers} key={`${path}.${index}`} />)}
        </div>
      )}
    </div>
  );
...
export function ScenarioTree({ value }) {
  ...
  return <div className="scenario-tree" data-testid="scenario-preview">{steps.map((step, index) => <ScenarioTreeNode step={step} path={String(index)} stepNumbers={stepNumbers} key={String(index)} />)}</div>;
}
