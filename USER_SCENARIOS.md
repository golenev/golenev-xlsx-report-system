...
- На главной странице доступен выбор директории с `allure-results`; после выбора папки UI отправляет мультипарт-запрос с параметром `isRegressRunning` и парсит отчёт.
- В обычном режиме загрузки новые тест-кейсы создаются/обновляются без проставления статусов прогона; ручные поля и сортировка работают как и при ручном вводе.
- В режиме запущенного регресса (`isRegressRunning=true`) из отчёта в колонку **Regress Run** подставляется `runStatus` (`PASSED`/`FAILED`/`SKIPPED`) для каждой строки.
- При совпадении `testId` с существующей строкой все поля перезаписываются данными из отчёта (с учётом правил `forceUpdate` и регресса).
...
- Пользователь может загрузить новые тест-кейсы, указав директорию с `allure-results`: сервис читает статусы тестов из отчёта и создаёт/обновляет строки.
- В режиме регресса (`isRegressRunning=true`) при загрузке allure-отчёта каждая строка получает значение `runStatus` (`PASSED`/`FAILED`/`SKIPPED`) в дополнительной колонке Regress Run; в обычном режиме поле не проставляется.
- При совпадении `testId` с уже существующей записью все поля тест-кейса перезаписываются данными из отчёта (с учётом правил `forceUpdate` и регресса).
