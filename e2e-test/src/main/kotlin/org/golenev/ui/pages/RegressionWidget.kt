...
class RegressionWidget {
    private val regressionActions =
        `$$`("div.regression-actions").name("Контейнеры действий regression widget, внутри которых находится кнопка старта регресса.")
    private fun regressionStartButton(): SelenideElement =
        regressionActions.findBy(text("Would you run regress")).name("Кнопка открытия формы запуска regression run.")
