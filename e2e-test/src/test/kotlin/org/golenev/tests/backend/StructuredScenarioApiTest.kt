...
class StructuredScenarioApiTest {
    ...
    fun createTestWithStructuredScenarioWithoutFormat() {
        ...
        step("Проверяем, что шаги вернулись объектами без format") {
            ...
            steps?.get(0)?.text.shouldBe("Первый шаг", "steps?.get(0)?.text не совпало с ожидаемым")
            steps?.get(0)?.attachments.shouldBe(emptyList(), "steps?.get(0)?.attachments не совпало с ожидаемым")
            steps?.get(1)?.number.shouldBe(2, "steps?.get(1)?.number не совпало с ожидаемым")
            steps?.get(1)?.text.shouldBe("Второй шаг", "steps?.get(1)?.text не совпало с ожидаемым")
        }
    }
    ...
    fun emptyStructuredScenarioStepsAreIgnored() {
        ...
        step("Проверяем, что пустой шаг не вернулся как полноценный шаг") {
            ...
            steps?.size.shouldBe(2, "steps?.size не совпало с ожидаемым")
            steps?.get(0)?.number.shouldBe(1, "steps?.get(0)?.number не совпало с ожидаемым")
            steps?.get(1)?.number.shouldBe(3, "steps?.get(1)?.number не совпало с ожидаемым")
        }
    }
    ...
    fun stringScenarioIsRejected() {
        ...
        step("Проверяем, что API отклоняет scenario в виде строки") {
            ...
            errorResponse.path.shouldBe("/api/tests", "errorResponse.path не совпало с ожидаемым")
        }
    }
